...
export class ReportsService {
  ...
  async getManagerStats(managerId: string) {
    const projects = await this.projectModel.find({ createdBy: managerId }).exec();
    ...
    const memberStats = await Promise.all(
      team.members.map(async (member: any) => {
        const userTasks = tasks.filter(
          (t) =>
            t.assignedTo &&
            (t.assignedTo as any).equals(member._id),
        );
  ...
  async getTeamWorkload(teamId: string) {
    ...
    team.members.forEach((member: any) => {
      const memberTasks = tasks.filter(
        (t) => t.assignedTo && (t.assignedTo as any).equals(member._id),
      );
